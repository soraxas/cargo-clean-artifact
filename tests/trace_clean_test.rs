...
"#,
    ...
    println!("Found {rlib_count} .rlib files and {rmeta_count} .rmeta files");
...
"#,
    ...
    assert!(
        artifacts_after.len() > artifacts_before.len(),
    ...
    println!(
        "Found {} serde artifacts that are now stale",
        serde_artifacts.len()
    );
    assert!(
        !serde_artifacts.is_empty(),
        "Should have stale serde artifacts"
    );
}
